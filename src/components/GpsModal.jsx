...
export default function GpsModal({ isOpen, onClose }) {
  ...
  const destinationQuery = encodeURIComponent('Fertivitro - Av. Indianópolis, 529 - Moema, São Paulo');
  const googleMapsUrl = `https://www.google.com/maps/dir/?api=1&destination=${destinationQuery}&destination_place_id=ChIJ3RHXZhZa5pQRyRreDpytyoU`;
  const wazeUrl = `https://waze.com/ul?q=${destinationQuery}&navigate=yes`;
  const wazeAppUrl = `waze://?q=${destinationQuery}&navigate=yes`;
        ...
        >
          ...
          <div className="text-center sm:text-left mb-5">
            ...
            <p className="text-xs text-slate-500 mt-1 flex items-center justify-center sm:justify-start gap-1">
              ...
              <span className="line-clamp-1">{destinationAddress}</span>
            </p>
          </div>
            ...
            >
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Navegar por endereço com alertas em tempo real
                  </p>
                </div>
              </div>
